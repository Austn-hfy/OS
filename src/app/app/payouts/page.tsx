...
export default async function PayoutsPage({ searchParams }: { searchParams: Promise<{ residency?: string }> }) {
  ...
  return <>
    <header className="page-header payout-page-header"><div><p className="eyebrow">{selected?.name ?? "HFY company"}</p><h1>Payouts</h1><p className="subhead"><strong>{formatMoney(readyTotal)}</strong> ready to pay across {readyRows.length} Assignment{readyRows.length === 1 ? "" : "s"}. Completed eligible work still enters this queue automatically.</p></div></header>
