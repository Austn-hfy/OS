...
export function CommittedPlanForm({
  ...
  return <form action={action} className="platform-plan-form">
    ...
    <div className="platform-plan-form-grid">
      <div className="field">
        <label htmlFor={`${residencyId}-talent-bucket`}>Talent bucket</label>
      ...
      <div className="field">
        <label htmlFor={`${residencyId}-house-bucket`}>House bucket</label>
      ...
      <div className="platform-plan-rate-summary">
        ...
        <small>{comped ? "Buckets stay stored for usage comparison" : "Same flat rate for Talent and House"}</small>
      </div>
      ...
      <div className="field platform-plan-reason"><label htmlFor={`${residencyId}-reason`}>Reason for manual change</label><input id={`${residencyId}-reason`} name="changeReason" type="text" minLength={3} maxLength={500} placeholder={`Confirmed bucket plan for ${residencyName}`} required /></div>
    </div>
    <p className="privacy-note">Saving changes only the Committed Plan. Live Usage never edits buckets, creates automatic overage charges, or restricts access.</p>
