...
export function InvoiceWorkspace({
  ...
  return <div className="invoice-workspace">
    ...
    </form> : <div className="table-wrap invoice-table"><table><thead><tr><th>Invoice</th><th>Type</th><th>Period</th><th>Client total</th><th>Internal talent cost</th><th>Internal margin</th><th>Status</th><th>Delivery</th><th>Action</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td><strong>{row.invoiceNumber}</strong></td><td>{row.kind === "custom" ? "Custom" : "Scheduled services"}</td><td>{date(row.billingPeriodStart)}–{date(row.billingPeriodEnd)}</td><td><strong>{money(row.totalCents)}</strong><div className="privacy-note">{row.kind === "scheduled_period" ? `${row.calculatedHours.toFixed(1)} billed hours` : "Custom line items"}</div></td><td>{money(row.talentCostCents)}</td><td>{money(row.grossMarginCents)}<div className="privacy-note">{row.marginPercentage === null ? "—" : `${row.marginPercentage.toFixed(1)}%`}</div></td><td><Status value={row.status} /></td><td>{row.autoSendInvoices ? (row.deliveryStatus ? <Status value={row.deliveryStatus} /> : "Awaiting approval") : <><Status value="manual" /><div className="privacy-note">{row.autoSendReason || "Manual send"}</div></>}</td><td className="invoice-action-cell">{row.status === "draft" ? <InvoiceApprovalButton invoiceId={row.id} autoSend={row.autoSendInvoices} /> : null}{row.pdfStoragePath && row.status !== "draft" ? <a className="button secondary" href={`/app/invoices/${row.id}/pdf`}>Download PDF</a> : null}{row.status === "approved" && row.deliveryStatus === "failed" ? <form action={retryInvoiceSendAction}><input name="invoiceId" type="hidden" value={row.id} /><button className="button" type="submit">Retry send</button></form> : null}</td></tr>)}</tbody></table>{!rows.length ? <div className="empty">No Invoices yet. Create the first Draft when you&apos;re ready.</div> : null}</div>}
