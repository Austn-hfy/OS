            ...
            {modal.type === "add" ? (
              addMode === "choose" ? <div className="quick-add-choice">
                ...
                {activeSuggestion?.existing ? <div className="quick-existing"><p>This slot is already scheduled.</p><button className="button" type="button" onClick={() => openExistingDaypart(activeSuggestion.daypartId, modal.date)}>View scheduled slot</button></div> : activeSuggestion ? <>
                  ...
                  {activeSuggestion.type === "dj_artist" ? <><div className="quick-assignment-heading"><div><strong>DJ assignments</strong><small>{activeSuggestion.defaultDjCount ? `Target: ${activeSuggestion.defaultDjCount}. ` : "No DJ target set. "}You can add one DJ or several; individual hours must cover the service window without overlapping.</small></div></div>
