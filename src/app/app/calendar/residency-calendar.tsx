...
function daypartOccurrenceDraftFromEvent(event: ResidencyEvent): DaypartOccurrenceEditDraft {
  return {
    start: minuteToClock(event.shiftStartMinute),
    end: minuteToClock(event.shiftEndMinute),
    notes: event.notes ?? "",
...
export function ResidencyCalendar({ residency, monthKey, calendarView = "month", weekStart, events, rooms, dayparts, talent, requestTalent = [], dateExceptions, residencyOptions, residencySelectionParam = "residency", initialEventId, initialBatchDaypartId, previewMode = false, fullProgramming = false, calendarBasePath = "/app/calendar", canManage = true }: ResidencyCalendarProps) {
  ...
  const editingDaypart = editingEvent?.daypartId ? dayparts.find((daypart) => daypart.id === editingEvent.daypartId) : undefined;
  const editingReusableTemplate = editingDaypart?.scheduleMode === "calendar_only";
  const datedRemovalLabel = editingReusableTemplate ? "Remove from calendar" : "Skip this date";
  const datedRemovalHelp = editingReusableTemplate ? "The reusable template stays saved in Day Parts." : "The standing Daypart remains active.";
  ...
  function openEvent(event: MonthCalendarEvent) {
    ...
    setDaypartOccurrenceEditDraft(residencyEvent?.recordType === "nonfinancial_occurrence" && residencyEvent.daypartId ? daypartOccurrenceDraftFromEvent(residencyEvent) : null);
  ...
  async function saveDaypartOccurrence() {
    if (!editingEvent?.daypartId || editingEvent.recordType !== "nonfinancial_occurrence" || !daypartOccurrenceEditDraft) return;
    setEditPending(true);
    try {
      const startMinute = clockToMinute(daypartOccurrenceEditDraft.start);
      const endMinute = resolveEndMinute(startMinute, daypartOccurrenceEditDraft.end);
      const formData = new FormData();
      formData.set("id", editingEvent.id);
      formData.set("startMinute", String(startMinute));
      formData.set("endMinute", String(endMinute));
      formData.set("notes", daypartOccurrenceEditDraft.notes);
      formData.set("programDetails", daypartOccurrenceEditDraft.programDetails);
      formData.set("manualHostName", daypartOccurrenceEditDraft.manualHostName);
  ...
  async function skipSelectedDate() {
    ...
    const calendarOnly = dayparts.find((daypart) => daypart.id === daypartId)?.scheduleMode === "calendar_only";
    const message = calendarOnly
      ? `Remove ${editingEvent?.title ?? "this activity"} from ${serviceDate}? The reusable template will stay saved in Day Parts.`
      : editingEvent
  ...
  const daypartOccurrenceEditor = editingEvent?.recordType === "nonfinancial_occurrence" && editingEvent.daypartId && daypartOccurrenceEditDraft ? <section className="one-time-record-editor daypart-occurrence-editor">
    ...
    {editingEvent.daypartType === "house_activity" ? <div className="quick-program-fields"><div className="field"><label>Program / activity details <span>optional</span></label><input value={daypartOccurrenceEditDraft.programDetails} onChange={(event) => setDaypartOccurrenceEditDraft({ ...daypartOccurrenceEditDraft, programDetails: event.target.value })} /></div><div className="field"><label>Host / guest name <span>optional</span></label><input value={daypartOccurrenceEditDraft.manualHostName} onChange={(event) => setDaypartOccurrenceEditDraft({ ...daypartOccurrenceEditDraft, manualHostName: event.target.value })} /></div></div> : null}
    <div className="field quick-booking-notes"><label>Notes <span>optional</span></label><textarea value={daypartOccurrenceEditDraft.notes} onChange={(event) => setDaypartOccurrenceEditDraft({ ...daypartOccurrenceEditDraft, notes: event.target.value })} /></div>
    <div className="replacement-actions"><button className="button" type="button" disabled={editPending} onClick={saveDaypartOccurrence}>{editPending ? "Saving…" : "Save date changes"}</button></div>
  </section> : null;
        ...
        <section className={`quick-modal ${modal.type === "edit" ? "quick-modal-edit" : ""} ${modal.type === "add" && addMode === "room" ? "quick-modal-room-picker" : ""}`} role="dialog" aria-modal="true" aria-labelledby="quick-modal-title">
          ...
          <div className="quick-modal-body">
            {modal.type === "add" ? (
              ...
              {editingEvent.daypartId ? <>{editingEvent.daypartType === "house_activity" ? <div className="quick-house-activity"><strong>{editingReusableTemplate ? "Reusable template scheduled" : "Tracking-only Daypart scheduled"}</strong><p>No payout or invoice records were created.</p></div> : null}{materializedOccurrenceTalentEditor}{daypartOccurrenceEditor}</> : oneTimeRecordEditor}
