  ...
  return <div className="artist-lookup-shell">
    ...
    <section className="artist-detail-panel">
      {!selected ? <div className="artist-detail-empty"><span>HFY</span><h2>Select an artist</h2><p>Choose someone from the roster to see what they are owed, upcoming bookings, contact information, and payment details.</p></div> : <>
        ...
        <section className="artist-owed-total"><span>Total outstanding owed</span><strong>{currency(selected.totalOutstandingOwedCents)}</strong><small>{selected.outstandingAssignments.length ? `${selected.outstandingAssignments.length} unpaid Assignment${selected.outstandingAssignments.length === 1 ? "" : "s"}` : "No ready-to-pay Assignments"}</small></section>

        <section className="artist-detail-section"><div className="artist-section-heading"><div><p className="eyebrow">Owed from</p><h3>Unpaid Assignments</h3></div><strong>{currency(selected.totalOutstandingOwedCents)}</strong></div>{selected.outstandingAssignments.length ? <div className="artist-owed-list">{selected.outstandingAssignments.map((assignment) => <article key={assignment.id}><div><strong>{assignment.residencyName}</strong><span>{assignment.shiftName} · {serviceDateLabel(assignment.serviceDate)}</span></div><strong>{currency(assignment.amountCents)}</strong></article>)}</div> : <p className="artist-section-empty">Nothing is currently outstanding for this artist.</p>}</section>
