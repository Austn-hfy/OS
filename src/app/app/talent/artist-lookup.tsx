  ...
  return <div className="artist-lookup-shell">
    ...
    <section className="artist-detail-panel">
      {!selected ? <div className="artist-detail-empty"><span>HFY</span><h2>Select an artist</h2><p>Choose someone from the roster to see what they are owed, upcoming bookings, contact information, and payment details.</p></div> : <>
        ...
        {selected.airtableImportedAt ? <section className="artist-detail-section"><div className="artist-section-heading"><div><p className="eyebrow">Historical snapshot</p><h3>Airtable record</h3></div></div><dl className="artist-definition-list"><div><dt>Total earnings (all time)</dt><dd>{currency(selected.legacyTotalEarningsCents)}</dd></div><div><dt>Imported</dt><dd>{new Date(selected.airtableImportedAt).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}</dd></div><div><dt>Upcoming-bookings snapshot</dt><dd>{selected.legacyUpcomingBookings || "None recorded at import"}</dd></div></dl></section> : null}
      </>}
    </section>
