  ...
  return <div className="artist-lookup-shell">
    ...
    <section className="artist-detail-panel">
      {!selected ? <div className="artist-detail-empty"><span>HFY</span><h2>{artists.length ? "Select an artist" : "Build your roster"}</h2><p>{artists.length ? "Choose someone from the roster to see what they are owed, upcoming bookings, Residency access, contact information, and payment details." : "Use New Artist to add the first person to Artist Lookup."}</p></div> : <>
        ...
        {currentResidency ? <section className="artist-residency-access"><div><p className="eyebrow">Residency artist</p><div className="artist-residency-chips"><span>{currentResidency.name}</span></div></div><small>Manage this list from Residency Setup.</small></section> : <section className="artist-residency-access"><div><p className="eyebrow">Residency access</p><div className="artist-residency-chips">{selected.approvedResidencies.length ? selected.approvedResidencies.map((residency) => <span key={residency.id}>{residency.name}</span>) : <small>Not currently available in any Residency.</small>}</div></div><button className="button secondary" type="button" onClick={openResidencyManager}>{selected.approvedResidencies.length ? "Manage Residencies" : "Add to Residency"}</button></section>}
