  ...
  return <div className="artist-lookup-shell">
    <aside className="artist-roster-panel">
      ...
      <div className="artist-roster-list">{filteredArtists.map((artist) => <div className={`artist-roster-row-wrap ${selectedId === artist.id ? "selected" : ""}`} key={artist.id}>
        ...
        <button className="artist-roster-row" type="button" onClick={() => { setSelectedId(artist.id); setEditing(false); }}>
          <span className="artist-roster-row-heading"><strong>{artist.stageName}</strong>{artist.totalOutstandingOwedCents > 0 ? <small className="artist-owed-chip">Owed <PrivateValue>{currency(artist.totalOutstandingOwedCents)}</PrivateValue></small> : null}</span>
    ...
    <section className="artist-detail-panel">
      {!selected ? <div className="artist-detail-empty"><span>HFY</span><h2>Select an artist</h2><p>Choose someone from the roster to see what they are owed, upcoming bookings, Residency access, contact information, and payment details.</p></div> : <>
        ...
        <section className="artist-owed-total"><span>Total outstanding owed</span><strong><PrivateValue>{currency(selected.totalOutstandingOwedCents)}</PrivateValue></strong><small>{selected.outstandingAssignments.length ? `${selected.outstandingAssignments.length} HFY OS unpaid Assignment${selected.outstandingAssignments.length === 1 ? "" : "s"}` : "No HFY OS ready-to-pay Assignments"}{selected.legacyOutstandingOwedCents ? <> · Includes <PrivateValue>{currency(selected.legacyOutstandingOwedCents)}</PrivateValue> imported from Airtable</> : null}</small></section>

        <section className="artist-detail-section"><div className="artist-section-heading"><div><p className="eyebrow">Owed from</p><h3>Unpaid Assignments</h3></div><strong><PrivateValue>{currency(selected.totalOutstandingOwedCents)}</PrivateValue></strong></div>{selected.outstandingAssignments.length ? <div className="artist-owed-list">{selected.outstandingAssignments.map((assignment) => <article key={assignment.id}><div><strong>{assignment.residencyName}</strong><span>{assignment.shiftName} · {serviceDateLabel(assignment.serviceDate)}</span></div><strong><PrivateValue>{currency(assignment.amountCents)}</PrivateValue></strong></article>)}</div> : null}{selected.legacyOutstandingOwedCents ? <article className="artist-legacy-financial"><div><strong>Airtable outstanding snapshot</strong><span><PrivateValue>{selected.legacyOwedFrom || "Itemized Airtable source was blank."}</PrivateValue></span></div><strong><PrivateValue>{currency(selected.legacyOutstandingOwedCents)}</PrivateValue></strong></article> : null}{!selected.outstandingAssignments.length && !selected.legacyOutstandingOwedCents ? <p className="artist-section-empty">Nothing is currently outstanding for this artist.</p> : null}</section>
        ...
        {selected.airtableImportedAt ? <section className="artist-detail-section"><div className="artist-section-heading"><div><p className="eyebrow">Historical snapshot</p><h3>Airtable record</h3></div></div><dl className="artist-definition-list"><div><dt>Total earnings (all time)</dt><dd><PrivateValue>{currency(selected.legacyTotalEarningsCents)}</PrivateValue></dd></div><div><dt>Airtable roster status</dt><dd>{selected.airtableRosterStatusLabel || "Blank"}</dd></div><div><dt>Airtable talent status</dt><dd>{selected.airtableTalentStatusLabel || "Blank"}</dd></div><div><dt>Imported</dt><dd>{new Date(selected.airtableImportedAt).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}</dd></div><div><dt>Upcoming-bookings snapshot</dt><dd>{selected.legacyUpcomingBookings || "None recorded at import"}</dd></div></dl></section> : null}
