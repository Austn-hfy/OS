...
export function ArtistLookup({ artists, residencies, currentResidency }: { artists: ArtistRow[]; residencies: ResidencyRow[]; currentResidency: ResidencyRow | null }) {
  ...
  return <TalentWorkspaceShell sidebar={<>
      ...
      {selectedIds.size ? <div className="artist-bulk-toolbar" aria-live="polite"><div><strong>{selectedIds.size} selected</strong><button type="button" onClick={() => setSelectedIds(new Set())}>Clear</button></div><div className="artist-bulk-actions">{view === "archived" ? <button type="button" onClick={() => runBulk("restore")} disabled={rosterPending}>Restore</button> : <><button type="button" onClick={() => runBulk("active")} disabled={rosterPending}>Set Active</button><button type="button" onClick={() => runBulk("inactive")} disabled={rosterPending}>Set Inactive</button><button type="button" onClick={() => runBulk("archive")} disabled={rosterPending}>Archive</button>{!currentResidency && residencies.length ? <span className="artist-bulk-residency"><select aria-label="Residency for selected artists" value={bulkResidencyId} onChange={(event) => setBulkResidencyId(event.target.value)}>{residencies.map((residency) => <option value={residency.id} key={residency.id}>{residency.name}</option>)}</select><button type="button" onClick={() => runBulk("add_to_residency", [...selectedIds], bulkResidencyId)} disabled={rosterPending || !bulkResidencyId}>Add to Residency</button></span> : null}</>}</div></div> : null}
      ...
      <div className="artist-roster-list">{filteredArtists.map((artist) => <div className={`artist-roster-row-wrap ${selectedId === artist.id ? "selected" : ""}`} key={artist.id}>
        ...
        <button className="artist-roster-row" type="button" onClick={() => { setSelectedId(artist.id); setEditing(false); }}>
          <span className="artist-roster-row-heading"><strong>{artist.stageName}</strong>{!currentResidency && artist.totalOutstandingOwedCents > 0 ? <small className="artist-owed-chip">Owed <PrivateValue>{currency(artist.totalOutstandingOwedCents)}</PrivateValue></small> : null}</span>
      ...
      {!selected ? <div className="artist-detail-empty"><span>HFY</span><h2>{artists.length ? "Select an artist" : "Build your roster"}</h2><p>{artists.length ? "Choose someone from the roster to see what they are owed, upcoming bookings, Residency assignments, contact information, and payment details." : "Use New Artist to add the first person to Artist Lookup."}</p></div> : <>
        ...
        {currentResidency ? <section className="artist-residency-access"><div><p className="eyebrow">Residency artist</p><div className="artist-residency-chips"><span>{currentResidency.name}</span></div></div><small>Explicitly assigned to this Residency.</small></section> : <section className="artist-residency-access"><div><p className="eyebrow">Residency assignments</p><div className="artist-residency-chips">{selected.approvedResidencies.length ? selected.approvedResidencies.map((residency) => <span key={residency.id}>{residency.name}</span>) : <small>Not assigned to a Residency.</small>}</div><small>{selected.exclusiveResidencyId ? `Exclusive to ${residencies.find((residency) => residency.id === selected.exclusiveResidencyId)?.name ?? "one Residency"}.` : "Shared — eligible to be assigned to multiple Residencies."}</small></div><button className="button secondary" type="button" onClick={openResidencyManager}>{selected.approvedResidencies.length ? "Manage Assignments" : "Add to Residency"}</button></section>}
        ...
        <section className="artist-detail-section"><div className="artist-section-heading"><div><p className="eyebrow">Owed from</p><h3>Unpaid Assignments</h3></div><strong><PrivateValue>{currency(selected.totalOutstandingOwedCents)}</PrivateValue></strong></div>{selected.outstandingAssignments.length ? <div className="artist-owed-list">{selected.outstandingAssignments.map((assignment) => <article key={assignment.id}><div><strong>{assignment.residencyName}</strong><span>{assignment.shiftName} · {serviceDateLabel(assignment.serviceDate)}</span></div><strong><PrivateValue>{currency(assignment.amountCents)}</PrivateValue></strong></article>)}</div> : null}{!currentResidency && selected.legacyOutstandingOwedCents ? <article className="artist-legacy-financial"><div><strong>Airtable outstanding snapshot</strong><span><PrivateValue>{selected.legacyOwedFrom || "Itemized Airtable source was blank."}</PrivateValue></span></div><strong><PrivateValue>{currency(selected.legacyOutstandingOwedCents)}</PrivateValue></strong></article> : null}{!selected.outstandingAssignments.length && (!selected.legacyOutstandingOwedCents || currentResidency) ? <p className="artist-section-empty">Nothing is currently outstanding for this artist{currentResidency ? ` in ${currentResidency.name}` : ""}.</p> : null}</section>
    ...
    {editing && selected ? <div className="artist-editor-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) setEditing(false); }}><aside className="artist-editor-drawer" role="dialog" aria-modal="true" aria-labelledby="artist-editor-title"><form action={editAction} className="artist-editor-form">
      ...
      <div className="artist-editor-scroll">
        ...
        <section><h3>Record status</h3><dl className="artist-definition-list"><div><dt>W-9</dt><dd>{selected.hasW9 ? "On file" : "Not on file"}</dd></div><div><dt>Documents</dt><dd>{selected.documentCount}</dd></div><div><dt>Residency assignments</dt><dd>{selected.approvedResidencies.map((residency) => residency.name).join(", ") || "None"}</dd></div></dl></section>
    ...
    {managingResidencies && selected ? <div className="artist-editor-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) setManagingResidencies(false); }}><aside className="artist-editor-drawer artist-residency-drawer" role="dialog" aria-modal="true" aria-labelledby="artist-residency-title"><div className="artist-editor-form">
      <header className="artist-editor-heading"><div><p className="eyebrow">Residency assignments</p><h2 id="artist-residency-title">{selected.stageName}</h2><p>Choose the Residency rosters where this artist should appear for new bookings.</p></div><button className="quick-modal-close" type="button" aria-label="Close Residency assignments" onClick={() => setManagingResidencies(false)}>×</button></header>
