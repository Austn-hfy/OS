  ...
  return (
    <div className={`shell ${!inResidency && view === "overview" ? "hfy-style-pilot" : ""}`}>
      <aside className="sidebar">
        ...
        <nav className="nav preview-nav">
          ...
          {(inPipeline
            ? (["leads"] as View[])
            : inResidency
            ? (["overview", "calendar", "talent", "payouts", "invoices", "setup"] as View[])
      ...
      <main className={`main ${view === "calendar" ? "calendar-main" : ""} ${inResidency && view === "calendar" ? "residency-calendar-main" : ""}`}>
        ...
        {inResidency && view === "talent" ? <PreviewArtistLookup residencyId={residency.id} residencyName={residency.name} /> : null}
...
function Calendar({ residency, shifts: residencyShiftRows, bookings: bookingRows, dayparts, onUpdate }: { residency: typeof residencies[number]; shifts: typeof shifts; bookings: typeof initialBookings; dayparts: PreviewDaypart[]; onUpdate: (id: string, status: BookingStatus) => void }) {
  ...
  const daypartSuggestions = dayparts.flatMap((daypart) => {
    ...
    return [{
      ...
      djs: rule.defaultDjCount,
    }];
  });
  const oneTimeDaypart = { id: "one-time", name: oneTimeSlot.name, room: oneTimeSlot.room, color: oneTimeSlot.color, defaultTalentRateCents: null, activeUntil: null, active: true, sortOrder: 0, rules: [], recurringToday: false, start: oneTimeSlot.start, end: oneTimeSlot.end, time: `${formatLocalMinute(clockToMinute(oneTimeSlot.start))}–${formatLocalMinute(resolveEndMinute(clockToMinute(oneTimeSlot.start), oneTimeSlot.end))}`, djs: 1 };
            ...
            {modal === "add" ? (
              addMode === "choose" ? <div className="quick-add-choice"><button className="quick-add-choice-card" type="button" onClick={() => { const next = daypartSuggestions.find((daypart) => daypart.id === activeDaypartId) ?? daypartSuggestions[0]; if (!next) return; setActiveDaypartId(next.id); setCustomStart(next.start); setCustomEnd(next.end); setCustomHours(!next.recurringToday); setAddMode("daypart"); }}><span>Existing Daypart</span><strong>Use a setup Daypart</strong><small>Choose any active Daypart and adjust its hours for this date.</small></button><button className="quick-add-choice-card one-time" type="button" onClick={() => { setActiveDaypartId("one-time"); setCustomStart(oneTimeSlot.start); setCustomEnd(oneTimeSlot.end); setCustomHours(true); setAddMode("one-time"); }}><span>One-time</span><strong>Create a one-time slot</strong><small>Add something unique without changing the standing schedule.</small></button></div> : !activeDaypart ? <div className="empty quick-empty">No active Daypart is available.</div> : (
                <form className="quick-book-form" onSubmit={submitAceDate}>
                  ...
                  <div className="quick-assignment-heading"><div><strong>DJ assignments</strong><small>{activeDaypart?.djs} usually scheduled · add one or several</small></div></div>
...
function Setup({ residency, dayparts, onDaypartsChange, defaultTalentRate, onDefaultTalentRateChange }: { residency: typeof residencies[number]; dayparts: PreviewDaypart[]; onDaypartsChange: (dayparts: PreviewDaypart[]) => void; defaultTalentRate: string; onDefaultTalentRateChange: (rate: string) => void }) {
  ...
  return <><header className="page-header card"><div><p className="eyebrow">{residency.name} · Preview</p><h1>Setup</h1><p className="subhead">Define recurring Dayparts, approved talent, and rate defaults for this Residency. Invoice delivery and billing configuration now live inside Invoices.</p></div></header><PreviewDaypartManager dayparts={dayparts} onChange={onDaypartsChange} /><section className="grid residency-grid"><form className="card selection-form residency-rate-editor" onSubmit={(event) => { event.preventDefault(); setSavedRate(defaultTalentRate); }}><div><p className="eyebrow">Residency default</p><h2>Talent rate</h2></div><div className="field"><label>Talent rate ($/hr)</label><input type="number" min="0" step="0.01" value={defaultTalentRate} onChange={(event) => onDefaultTalentRateChange(event.target.value)} required /><small>Used after Assignment and Daypart overrides.</small></div><p className="privacy-note">Changing this affects new Assignments only. Existing payout amounts are never recalculated.</p>{savedRate === defaultTalentRate ? <p className="muted">Current default: ${Number(savedRate || 0).toFixed(2)} / hour</p> : <p className="draft-notice">Save this new default before leaving Setup.</p>}<button className="button" type="submit">Save talent default</button></form></section></>;
}
