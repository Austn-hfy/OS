...
export default async function ResidencyPlatformBillingPage({ searchParams }: { searchParams: Promise<{ stripe?: string; card?: string; liveBilling?: string }> }) {
  ...
  return <WorkspaceSurface className="residency-workspace-surface workspace-surface-settings workspace-surface-platform-billing">
    ...
    {subscription ? <>
      {subscription.comped ? <section className="platform-founding-state active" role="status"><div><strong>Complimentary Platform plan</strong><span>Your Residency is permanently comped at $0. No commitment tier or payment method is required.</span></div><span className="status active">COMPED</span></section> : null}
      ...
      <section className="platform-billing-summary">
        ...
        {subscription.comped ? <article className="card"><small>Commitment tier</small><strong>Not required</strong><span>Outside the commitment ladder</span></article> : <article className="card"><small>Next charge</small><strong>{date(subscription.nextChargeAt ?? subscription.renewsOn)}</strong><span className={`status ${subscription.status}`}>{subscription.status.replaceAll("_", " ")}</span></article>}
      </section>
      <section className="card platform-plan-breakdown"><div><p className="eyebrow">Committed Plan</p><h2>Subscription details</h2><p>Live Usage is tracked separately and never changes this bill automatically.</p></div><dl><div><dt>Talent sessions</dt><dd>{subscription.talentProgramSessions} × {money(subscription.talentSessionUnitAmountCents)}</dd></div><div><dt>House programs</dt><dd>{subscription.housePrograms} × {money(subscription.houseProgramUnitAmountCents)}</dd></div><div><dt>Monthly one-offs included</dt><dd>{subscription.oneOffAllowance}</dd></div><div><dt>Plan dates</dt><dd>{date(subscription.startsOn)}–{date(subscription.renewsOn)}</dd></div><div><dt>Monthly plan</dt><dd>{money(subscription.monthlyAmountCents)}</dd></div></dl></section>
