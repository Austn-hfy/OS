...
export default async function ResidencyPlatformBillingPage({ searchParams }: { searchParams: Promise<{ stripe?: string; card?: string; liveBilling?: string }> }) {
  ...
  return <WorkspaceSurface className="residency-workspace-surface workspace-surface-settings workspace-surface-platform-billing">
    ...
    {subscription ? <>
      {subscription.comped ? <section className="platform-comped-state active" role="status"><div><strong>Complimentary Platform plan</strong><span>Your Residency is permanently comped at $0. Its selected buckets still determine usage capacity.</span></div><span className="status active">COMPED</span></section> : null}
      {subscription.paymentFailedAt ? <section className="platform-payment-failed-inline" role="alert"><strong>Payment failed</strong><span>{subscription.paymentFailureMessage || "Stripe could not collect the latest payment."} Your portal remains fully available.</span>{subscription.comped ? null : <PlatformBillingActionForm action={updateResidencyPlatformCardAction} label="Update test card" pendingLabel="Opening card update…" />}</section> : null}
      <section className="platform-billing-summary">
        <article className="card"><small>Current plan · revision {subscription.revision}</small><strong>{money(subscription.effectiveMonthlyAmountCents)} / month</strong><span>{subscription.term === "annual" ? `Annual · ${money(subscription.termChargeAmountCents)} paid upfront` : `Month-to-month · ${money(subscription.termChargeAmountCents)} per charge`}</span></article>
        {subscription.comped ? <article className="card"><small>Payment method</small><strong>Not required</strong><span>Permanent complimentary status</span></article> : <article className="card"><small>Card on file</small><strong>{subscription.cardLast4 ? `${subscription.cardBrand} •••• ${subscription.cardLast4}` : "No card on file"}</strong><span>Managed securely through Stripe</span></article>}
      ...
      <section className="card platform-plan-breakdown"><div><p className="eyebrow">Committed Plan</p><h2>Subscription details</h2><p>Live Usage is tracked separately and never changes this bill automatically.</p></div><dl><div><dt>Talent bucket</dt><dd>{subscription.talentBucketSize} slots × {money(subscription.slotUnitAmountCents)}</dd></div><div><dt>House bucket</dt><dd>{subscription.houseBucketSize} slots × {money(subscription.slotUnitAmountCents)}</dd></div><div><dt>Billing term</dt><dd>{subscription.term === "annual" ? "Annual · 25% off" : "Month-to-month"}</dd></div><div><dt>Plan dates</dt><dd>{date(subscription.startsOn)}–{date(subscription.renewsOn)}</dd></div><div><dt>Base monthly plan</dt><dd>{money(subscription.baseMonthlyAmountCents)}</dd></div>{subscription.term === "annual" ? <div><dt>Annual upfront total</dt><dd>{money(subscription.termChargeAmountCents)}</dd></div> : null}</dl></section>
      ...
      {subscription.comped ? null : <section className="platform-card-actions"><PlatformBillingActionForm action={subscription.stripeSubscriptionId ? updateResidencyPlatformCardAction : startResidencyPlatformCheckoutAction} label={subscription.stripeSubscriptionId ? "Update test card" : "Add test card & start subscription"} pendingLabel={subscription.stripeSubscriptionId ? "Opening card update…" : "Starting Checkout…"} /></section>}
