...
export default async function ResidencyPlatformBillingPage({ searchParams }: { searchParams: Promise<{ stripe?: string; card?: string; liveBilling?: string; annualSwitch?: string }> }) {
  ...
  return <WorkspaceSurface className="residency-workspace-surface workspace-surface-settings workspace-surface-client-settings workspace-surface-platform-billing">
    ...
    <nav className="settings-tabs" aria-label="Settings sections"><Link href="/residency/settings">Account</Link><Link className="active" aria-current="page" href="/residency/settings/billing">Billing</Link></nav>
    <div className="settings-page-body settings-billing-body">
    ...
    {subscription ? <>
      ...
      <section className="card platform-plan-and-usage">
        <div className="platform-committed-plan-section">
          <div className="platform-plan-section-heading">
            <div><p className="eyebrow">Committed Plan</p><h2>Subscription details</h2><p>Live Usage is tracked separately and never changes this bill automatically.</p></div>
            {subscription.term === "annual" ? <span className="platform-plan-term-badge active">Annual · 25% off</span> : subscription.pendingAnnualChange ? <span className="platform-plan-term-badge scheduled">Annual scheduled</span> : null}
          </div>
          <dl className="platform-plan-facts">
            <div><dt>Talent capacity</dt><dd>{subscription.talentBucketSize} slots × {money(subscription.slotUnitAmountCents)}</dd></div>
            <div><dt>House capacity</dt><dd>{subscription.houseBucketSize} slots × {money(subscription.slotUnitAmountCents)}</dd></div>
            <div><dt>Billing term</dt><dd>{subscription.term === "annual" ? "Annual" : subscription.pendingAnnualChange ? `Month-to-month · Annual starts ${date(subscription.pendingAnnualChange.startsOn)}` : "Month-to-month"}</dd></div>
            <div><dt>Plan dates</dt><dd>{date(subscription.startsOn)}–{date(subscription.renewsOn)}</dd></div>
            <div><dt>Base monthly plan</dt><dd>{money(subscription.baseMonthlyAmountCents)}</dd></div>
            {subscription.term === "annual" ? <div><dt>Annual upfront total</dt><dd>{money(subscription.termChargeAmountCents)}</dd></div> : null}
          </dl>
