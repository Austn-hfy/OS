...
export default async function ResidencyPlatformBillingPage({ searchParams }: { searchParams: Promise<{ stripe?: string; card?: string; liveBilling?: string }> }) {
  ...
  return <WorkspaceSurface className="residency-workspace-surface workspace-surface-settings workspace-surface-platform-billing">
    ...
    {subscription ? <>
      ...
      <section className="card platform-plan-breakdown"><div><p className="eyebrow">Committed Plan</p><h2>Subscription details</h2><p>Live Usage is tracked separately and never changes this bill automatically.</p>{subscription.term === "month_to_month" && !subscription.comped ? <div className="platform-annual-switch"><strong>Save 25% by paying annually</strong><span>Your capacity stays the same. The annual term begins at your next renewal.</span><PlatformBillingActionForm action={switchResidencyPlatformPlanToAnnualAction} label="Switch to annual" pendingLabel="Scheduling annual billing…" /></div> : null}</div><dl><div><dt>Talent capacity</dt><dd>{subscription.talentBucketSize} slots × {money(subscription.slotUnitAmountCents)}</dd></div><div><dt>House capacity</dt><dd>{subscription.houseBucketSize} slots × {money(subscription.slotUnitAmountCents)}</dd></div><div><dt>Billing term</dt><dd>{subscription.term === "annual" ? "Annual · 25% off" : "Month-to-month"}</dd></div><div><dt>Plan dates</dt><dd>{date(subscription.startsOn)}–{date(subscription.renewsOn)}</dd></div><div><dt>Base monthly plan</dt><dd>{money(subscription.baseMonthlyAmountCents)}</dd></div>{subscription.term === "annual" ? <div><dt>Annual upfront total</dt><dd>{money(subscription.termChargeAmountCents)}</dd></div> : null}</dl></section>
