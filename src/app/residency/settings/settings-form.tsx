...
export function ResidencySettingsForm({ settings }: { settings: Settings }) {
  ...
  return <form action={action} className="card residency-client-settings-form">
    <ResidencySurfaceCard className="settings-account-section">
      <ResidencySectionHeader eyebrow="Account" title="Residency details" description="Basic information used throughout this Residency workspace." />
      <div className="settings-fields"><div className="field"><label>Residency name</label><input name="name" defaultValue={settings.name} required /></div><div className="field"><label>City / State</label><input name="cityState" defaultValue={settings.cityState} placeholder="Palm Springs, CA" /></div><div className="field wide"><label>Timezone</label><input name="timezone" defaultValue={settings.timezone} required /></div></div>
      {state.status !== "idle" ? <p className={state.status === "error" ? "error settings-account-status" : "success settings-account-status"} aria-live="polite">{state.message}</p> : null}
      <footer className="settings-account-actions"><span>These details identify the Residency throughout HFY OS.</span><button className="button" type="submit" disabled={pending}>{pending ? "Saving…" : "Save Settings"}</button></footer>
