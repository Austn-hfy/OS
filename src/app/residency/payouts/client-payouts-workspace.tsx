...
export function ClientPayoutsWorkspace({ rows, residencyName, timeZone }: { rows: Row[]; residencyName: string; timeZone: string }) {
  ...
  return <PayoutWorkspaceFrame
    ...
    filters={<section className="payout-filter-bar" aria-label="Payout filters">
      <div className="field payout-search"><label htmlFor="client-payout-artist-search">Artists</label><input id="client-payout-artist-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search artist or assignment" /></div>
      <div className="payout-sort-controls"><div className="field payout-sort"><label htmlFor="client-payout-sort">Sort by</label><select id="client-payout-sort" value={sortField} onChange={(event) => setSortField(event.target.value as SortField)}><option value="date">Service date</option><option value="amount">Amount owed</option></select></div><button className="payout-sort-direction" type="button" aria-label={`Sort ${sortDirection === "asc" ? "descending" : "ascending"}`} onClick={() => setSortDirection((current) => current === "asc" ? "desc" : "asc")}><span aria-hidden="true">{sortDirection === "asc" ? "↑" : "↓"}</span>{sortDirection === "asc" ? "Ascending" : "Descending"}</button></div>
