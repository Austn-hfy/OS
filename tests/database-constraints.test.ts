...
beforeAll(async () => {
  ...
  const platformBillingBucketsV14 = await readFile(new URL("../drizzle/0051_platform_billing_buckets_v14.sql", import.meta.url), "utf8");
  ...
  await database.exec(`
    ...
    VALUES ('${ids.platformSubscription}', '${ids.residencyA}', 'active', 'annual', 11, 6000, 6, 6000);
    INSERT INTO platform_subscription_revisions
      (platform_subscription_id, residency_id, revision, cadence, talent_program_sessions, talent_session_unit_amount_cents,
       house_programs, house_program_unit_amount_cents, one_off_allowance, unit_amount_cents, starts_on, renews_on, change_reason)
    VALUES
      ('${ids.platformSubscription}', '${ids.residencyA}', 1, 'annual', 11, 6000, 6, 6000, 0, 6000,
       '2027-01-01', '2027-12-31', 'Legacy migration fixture');
  `);
  await database.exec(platformBillingBucketsV14.replaceAll("--> statement-breakpoint", ""));
});
...
describe("database replacements for Airtable audit formulas", () => {
  ...
  it("defaults permanent comp status to off", async () => {
  ...
  it("backfills legacy rows upward and enforces v14 buckets, rate, and refund formula", async () => {
    const stored = await database.query<{ term: string; talent_bucket_size: number; house_bucket_size: number; slot_unit_amount_cents: number }>(`
      SELECT term, talent_bucket_size, house_bucket_size, slot_unit_amount_cents
      FROM platform_subscriptions WHERE id = '${ids.platformSubscription}';
    `);
    expect(stored.rows[0]).toEqual({ term: "annual", talent_bucket_size: 20, house_bucket_size: 10, slot_unit_amount_cents: 3000 });
    const revision = await database.query<{ term: string; talent_bucket_size: number; house_bucket_size: number }>(`
      SELECT term, talent_bucket_size, house_bucket_size FROM platform_subscription_revisions
      WHERE platform_subscription_id = '${ids.platformSubscription}' AND revision = 1;
    `);
    expect(revision.rows[0]).toEqual({ term: "annual", talent_bucket_size: 20, house_bucket_size: 10 });

    await expect(database.exec(`
      UPDATE platform_subscriptions SET talent_bucket_size = 12 WHERE id = '${ids.platformSubscription}';
    `)).rejects.toThrow(/talent_bucket_valid/);
    await expect(database.exec(`
      UPDATE platform_subscriptions SET house_bucket_size = 0 WHERE id = '${ids.platformSubscription}';
    `)).rejects.toThrow(/house_bucket_valid/);
    await expect(database.exec(`
      UPDATE platform_subscriptions SET slot_unit_amount_cents = 6000 WHERE id = '${ids.platformSubscription}';
    `)).rejects.toThrow(/slot_rate_fixed/);

    await database.exec(`
      INSERT INTO platform_subscription_refunds
        (platform_subscription_id, residency_id, source_revision, changed_at, total_annual_payment_cents,
         full_monthly_amount_cents, months_used, refund_amount_cents)
      VALUES
        ('${ids.platformSubscription}', '${ids.residencyA}', 1, '2027-04-01T00:00:00Z', 810000, 90000, 4, 450000);
    `);
    await expect(database.exec(`
      INSERT INTO platform_subscription_refunds
        (platform_subscription_id, residency_id, source_revision, changed_at, total_annual_payment_cents,
         full_monthly_amount_cents, months_used, refund_amount_cents)
      VALUES
        ('${ids.platformSubscription}', '${ids.residencyA}', 1, '2027-04-01T00:00:00Z', 810000, 90000, 4, 449999);
    `)).rejects.toThrow(/values_valid/);
  });
  ...
  it("keeps Platform subscription invoices in a Residency-scoped ledger", async () => {
    ...
    await expect(database.exec(`
      INSERT INTO platform_usage_snapshots
        (residency_id, platform_subscription_id, snapshot_date, period_start, period_end, talent_sessions, house_programs)
      VALUES ('${ids.residencyB}', '${ids.platformSubscription}', '2026-09-04', '2026-09-01', '2026-09-30', 8, 3);
    `)).rejects.toThrow(/must match its subscription Residency/);
    await expect(database.exec(`
