...
Vercel stores production and Preview migration credentials separately. `MIGRATION_DATABASE_URL` is restricted to Production and the `staging` Preview branch; `MIGRATION_DATABASE_SESSION_URL` is configured separately for Production and Preview. The first uses Supabase's direct Postgres endpoint. The second uses the shared session pooler on port `5432` with the same dedicated migration role, database, and project. The runner validates that identity pair before connecting, relies on PostgreSQL authentication to validate the session secret, and skips every non-staging Preview branch before reading either connection. It may use the session credential only when the direct endpoint fails for a genuine network-availability reason. `DATABASE_URL` is never a migration credential or fallback, and transaction pooling on port `6543` is never used for migrations.
