...
Vercel stores `MIGRATION_DATABASE_URL` and `MIGRATION_DATABASE_SESSION_URL` as Sensitive variables scoped separately to Production and to the `staging` Preview branch; they are deliberately absent from every other Preview branch. The first uses Supabase's direct Postgres endpoint. The second uses the shared session pooler on port `5432` with the same dedicated migration role, password, database, and project. The runner validates that identity pair before connecting and may use the session credential only when the direct endpoint fails for a genuine network-availability reason. `DATABASE_URL` is never a migration credential or fallback, and transaction pooling on port `6543` is never used for migrations.
