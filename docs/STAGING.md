...
Vercel stores `MIGRATION_DATABASE_URL` separately for Production and for the `staging` Preview branch. It is the only migration credential and uses Supabase's direct Postgres endpoint. The runner validates the allowlisted project, then derives a session-pooler fallback from that same URL by retaining its role, password, database, and options; adding Supabase's required project suffix to the role; and replacing only the host and port with the project's known session-pooler host on port `5432`. The allowlisted production and staging pooler hosts are kept in code, so no second manually entered migration credential is used. The runner skips every non-staging Preview branch before reading the credential and uses the derived fallback only when the direct endpoint fails for a genuine network-availability reason. `DATABASE_URL` is never a migration credential or fallback, and transaction pooling on port `6543` is never used for migrations.
